...
use crate::bus::{I2cMaster, I2cMessage};
...
impl DesignWareI2c {
    ...
    fn wait_tx_fifo_not_full(&self) -> bool {
        for _ in 0..POLL_LIMIT {
    ...
    fn wait_rx_fifo_not_empty(&self) -> Result<u32> {
        for _ in 0..POLL_LIMIT {
    ...
    fn ensure_fast_mode_timing(&self) {
    ...
    fn transfer(&self, target_addr: u8, messages: &mut [I2cMessage<'_>]) -> Result<()> {
        let total = messages
            .iter()
            .map(|msg| match msg {
    ...
    fn transfer_inner(&self, messages: &mut [I2cMessage<'_>]) -> Result<()> {
...
impl I2cMaster for DesignWareI2c {
    fn transfer(&self, address: u8, msgs: &mut [I2cMessage]) -> Result<()> {
